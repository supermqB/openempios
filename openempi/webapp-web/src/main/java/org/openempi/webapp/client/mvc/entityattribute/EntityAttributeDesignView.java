...
public class EntityAttributeDesignView extends View
{
    ...
    private Frame exportViewFrame = null;
...
//	      	store.add(entity);
			...
			if (entityDesignDialog != null && entityDesignDialog.isVisible()) {
				entityDesignDialog.close();
			}

	        if (importDialog != null && importDialog.isVisible()) {
	            importDialog.close();
	        }
		...
		toolBar.add(new Button(" Add Entity ", IconHelper.create("images/entity_add.png"), new SelectionListener<ButtonEvent>() {
	          ...
	          public void componentSelected(ButtonEvent ce) {
				  ...
				  readOnlyFields(false);
	      		  ...
	      		  versionSpin.hide();
	              entityDesignDialog.setHeight(540);
	          }
	    }));
...
//		      		  for( EntityAttributeWeb entityAttribute : removeEntity.getAttributes()) {
        ...
        toolBar.add(new Button(" Export Entity ", IconHelper.create("images/export.png"), new SelectionListener<ButtonEvent>() {
            ...
            public void componentSelected(ButtonEvent ce) {
                ...
                EntityWeb exportEntity = grid.getSelectionModel().getSelectedItem();
                if (exportEntity == null) {
                      Info.display("Information", "You must first select a field before pressing the \"Export Entity\" button.");
                      return;
                }

                String url = GWT.getHostPageBaseURL() + "entityDownloadService?entityName=" + exportEntity.getName()
                                + "&entityVersionId=" + exportEntity.getEntityVersionId().toString();
                exportViewFrame.setUrl(url);
            }
        }));
        ...
        exportViewFrame = new Frame();
        exportViewFrame.setSize("0", "0");
	...
	private void buildEntityDesignDialog() {
		...
		entityDesignDialog.setWidth(1060);
		entityDesignDialog.setHeight(565);
		...
		entityDesignDialog.getButtonById(Dialog.OK).addSelectionListener(new SelectionListener<ButtonEvent>() {
	          ...
	          public void componentSelected(ButtonEvent ce) {
				...
				if (addOrEditOrDeleteMode.equals(ADD_ENTITY)) {  // Add
                    ...
                    if (addingEntity.getAttributes().size() == 0) {
                        MessageBox.alert("Information", "" +
                                "No any attribute has been designed in the Entity you are adding", null);
                        return;
                    }

					// check Searchable attribute
					boolean hasSearchable = false;
                    for (EntityAttributeWeb attribute : addingEntity.getAttributes()) {
                    ...
                    if (!hasSearchable) {
                        MessageBox.alert("Information", "" +
                        		"Ensure that at least one attribute has been designed as Searchable in the Entity you are adding", null);
                        return;
                    }
                    ...
                    if (updatingEntity.getAttributes().size() == 0) {
                        MessageBox.alert("Information", "" +
                                "No any attribute has been designed in the Entity you are editing", null);
                        return;
                    }
                    ...
                    if (!hasSearchable) {
                        MessageBox.alert("Information", "" +
                        		"Ensure that at least one attribute has been designed as Searchable in the Entity you are editing", null);
                        return;
                    }
		...
		entityDesignDialog.getButtonById(Dialog.CANCEL).addSelectionListener(new SelectionListener<ButtonEvent>() {
	          ...
	          public void componentSelected(ButtonEvent ce) {
	        	  ...
	        	  if (addOrEditOrDeleteMode.equals(EDIT_ENTITY)) { 
	        	      controller.handleEvent(new AppEvent(AppEvents.EntitiesRequest));
	        	  }
	          }
	    });
	...
	private ContentPanel setupAttributePanel(String title) {
		...
		toolBar.add(new Button("Add Attribute", IconHelper.create("images/database_add.png"), new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
                ...
                attributeSearchableCheckBox.setValue(false);
                attributeCaseInsensitiveCheckBox.setValue(false);
				validationStore.removeAll();
		...
		toolBar.add(new Button("Edit Attribute", IconHelper.create("images/database_edit.png"), new SelectionListener<ButtonEvent>() {
			...
			public void componentSelected(ButtonEvent ce) {
                ...
                attributeSearchableCheckBox.setValue(editField.getSearchable());
                attributeCaseInsensitiveCheckBox.setValue(editField.getCaseInsensitive());
	    ...
	    CheckColumnConfig pSearchable = new CheckColumnConfig("searchable", "Searchable", 80);
	    CheckColumnConfig pCaseInsensitive = new CheckColumnConfig("caseInsensitive", "Case Insensitive", 100);
	    CellEditor checkBoxIndexedEditor = new CellEditor(new CheckBox());
	    pIndexed.setEditor(checkBoxIndexedEditor);
        CellEditor checkBoxSearchableEditor = new CellEditor(new CheckBox());
...
//TODO:
	    ...
	    pSearchable.setSortable(false);
	    pCaseInsensitive.setSortable(false);
		...
		// config.add(pDisplayOrder);
		config.add(pIndexed);
	    config.add(pSearchable);
	    config.add(pCaseInsensitive);
	    ...
	    attributeGrid.addPlugin(pSearchable);
	    attributeGrid.addPlugin(pCaseInsensitive);
...
*/
	...
	private EntityAttributeWeb copyEntityAttributeFromGUI(EntityAttributeWeb updateEntityAttribute) {
        ...
        entityAttribute.setSearchable(attributeSearchableCheckBox.getValue());
        entityAttribute.setCaseInsensitive(attributeCaseInsensitiveCheckBox.getValue());
...
//			        	attributeStore.add(attribute);
	    ...
	    CheckBoxGroup checkIndexedEnableGroup = new CheckBoxGroup();
	    checkIndexedEnableGroup.setFieldLabel("Indexed");
	    checkIndexedEnableGroup.add(attributeIndexedCheckBox);

	    attributeSearchableCheckBox = new CheckBox();
	    attributeSearchableCheckBox.setBoxLabel("");
        CheckBoxGroup checkSearchableEnableGroup = new CheckBoxGroup();
        checkSearchableEnableGroup.setFieldLabel("Searchable");
        checkSearchableEnableGroup.add(attributeSearchableCheckBox);

        attributeCaseInsensitiveCheckBox = new CheckBox();
        attributeCaseInsensitiveCheckBox.setBoxLabel("");
        CheckBoxGroup checkCaseInsEnableGroup = new CheckBoxGroup();
        checkCaseInsEnableGroup.setFieldLabel("Case Insensitive");
        checkCaseInsEnableGroup.add(attributeCaseInsensitiveCheckBox);
		...
		// cp.add(sliderField);
		cp.add(checkIndexedEnableGroup);
        cp.add(checkSearchableEnableGroup);
        cp.add(checkCaseInsEnableGroup);
		cp.add(formContainer);
