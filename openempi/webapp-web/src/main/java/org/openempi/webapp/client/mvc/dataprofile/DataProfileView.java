...
public class DataProfileView extends View
{
     ...
     *
     * public void chartClick(ChartEvent ce) { Info.display("Chart Clicked", "You selected ", "" + ce.getValue()); } };
     */
        ...
        if (event.getType() == AppEvents.DataProfileView) {
            ...
            for (DataProfileAttributeValueWeb attributeValueWeb : attributeValueList) {
                ...
                if (currentAttribute.getRowCount() > 0) {
                    attributeValueWeb.set("percent",
                            (0.0 + attributeValueWeb.getFrequency()) / currentAttribute.getRowCount());

                    cumulative += (0.0 + attributeValueWeb.getFrequency()) / currentAttribute.getRowCount();
                    attributeValueWeb.set("cumulative", cumulative);

                } else {
                    attributeValueWeb.set("percent", 0.0);
                    attributeValueWeb.set("cumulative", cumulative);
                }
            }
    ...
    private void setupProfileAttributesGrid() {
        ...
        configs.add(column);
        /*
         * column = new ColumnConfig(); column.setId("averageValue"); column.setHeader("Average Value");
         * column.setWidth(90); configs.add(column);
         *
         * column = new ColumnConfig(); column.setId("minimumValue"); column.setHeader("Minimum Value");
         * column.setWidth(90); configs.add(column);
         *
                ...
                new Listener<SelectionChangedEvent<DataProfileAttributeWeb>>()
                {
                    public void handleEvent(SelectionChangedEvent<DataProfileAttributeWeb> be) {
                        ...
                        if (currentAttribute != null) {
                            ...
                            controller.handleEvent(new AppEvent(AppEvents.DataProfileAttributeValueRequest,
                                    currentAttribute));
        ...
        reloadDataProfileButton = new Button(" Refresh Data Profile", IconHelper.create("images/arrow_refresh.png"),
    ...
    private ChartModel getPieChartModel(List<DataProfileAttributeValueWeb> attributeValueList) {
        ChartModel cm = new ChartModel(currentAttribute.getAttributeName(),
        ...
        pie.setColours("#ff0000", "#00ffff", "#0000ff", "#add8e6", "#800080", "#808080", "#00ff00", "#ff00ff",
        ...
        for (DataProfileAttributeValueWeb attributeValueWeb : attributeValueList) {
            // Info.display("Information", ""+attributeValueWeb.getAttributeValue());
            pie.addSlices(new PieChart.Slice(attributeValueWeb.getFrequency(), attributeValueWeb.getAttributeValue(),
                    attributeValueWeb.getAttributeValue()));
    ...
    private ChartModel getBarChartCountModel(DataProfileAttributeWeb attribute) {
        ...
        /*
         * String attributeFullName = ""; for (ModelPropertyWeb nameField : fieldNames) {
         * if(nameField.getName().equals(attribute.getAttributeName()) ) { attributeFullName =
         * nameField.getDescription(); } } ChartModel cm = new ChartModel(attributeFullName,
         * "font-size: 12px; font-family:      Verdana; text-align: center;");
         */
        ChartModel cm = new ChartModel(attribute.getAttributeName(),
                "font-size: 12px; font-family:      Verdana; text-align: center;");
        ...
        bchart.addBars(new BarChart.Bar((attribute.getDistinctCount() != null) ? attribute.getDistinctCount() : 0,
                "#00ff00"));
        bchart.addBars(new BarChart.Bar((attribute.getUniqueCount() != null) ? attribute.getUniqueCount() : 0,
                "#ff0000"));
        bchart.addBars(new BarChart.Bar((attribute.getDuplicateCount() != null) ? attribute.getDuplicateCount() : 0,
                "#333ccc"));
    ...
    private ChartModel getBarChartLengthModel(DataProfileAttributeWeb attribute) {
        ...
        // different color for different bars
        bchart.addBars(new BarChart.Bar((attribute.getAverageLength() != null) ? attribute.getAverageLength() : 0,
                "#ffff00"));
        bchart.addBars(new BarChart.Bar((attribute.getMinimumLength() != null) ? attribute.getMinimumLength() : 0,
                "#0000ff"));
        bchart.addBars(new BarChart.Bar((attribute.getMaximumLength() != null) ? attribute.getMaximumLength() : 0,
                "#00ff00"));
