...
/**
 * This class processes PIX Feed message in HL7 v2.3.1 format. It handles the PIX Feed transaction (including also PIX
 * Update and PIX Merge transactions) of the PIX profile. The supported message type includes ADT^A01, ADT^A04, ADT^A05,
 * ADT^A08 and ADT^A40.
 *
 * 
 */
class PixFeedHandler extends BaseHandler implements ReceivingApplication
{
    ...
    /**
     * Processes the incoming PIX Feed Message. Valid messages are ADT^A01, ADT^A04, ADT^A05, ADT^A08 and ADT^A40.
     * 
     * @param msgIn
     ...
     * @throws IOException
     */
    public Message processMessage(Message msgIn, Map<String, Object> theMetadata)
        ...
        try {
            ...
            if (msgIn instanceof ADT_A01 || // Admission of in-patient into a facility
            ...
            } else if (msgIn instanceof ca.uhn.hl7v2.model.v25.message.ADT_A01 || // Admission of in-patient into a
                                                                                  // facility
                ...
                throw new ReceivingApplicationException(errorMsg);
            }
    ...
    private Message processUpdate(Message msgIn) throws PixPdqException, HL7Exception {
        ...
        try {
            ...
            if (actor.getXdsRegistryConnection() != null && matchingList != null) {
                ...
                String XDSAffinityDomain = actor.getXdsRegistryConnection()
                        .getProperty("XDSAffinityDomainPatientIdetifier");
                ...
                for (List<PatientIdentifier> matching : matchingList) {
                    ...
                    if (matching.contains(patientId)) {
                        ...
                        if (XADNotification) {
                            IPixUpdateNotificationRequest matchingRequest = new PixXADUpdateNotificationRequest(actor,
                                    matching, oldXADMatchings);
                            PixUpdateNotifier.getInstance().accept(matchingRequest);
                        }
                    }
                }
     ...
     *             if something is wrong with HL7 message
     */
    private Message processMerge(Message msgIn) throws PixPdqException, HL7Exception {
    ...
    private ACK initAcknowledgment(HL7Header hl7Header) throws HL7Exception, PixPdqException {
        ...
        try {
            ...
            HL7v231.populateMSH(reply.getMSH(), "ACK", event, getMessageControlId(), serverApplication, serverFacility,
     ...
     * @return <code>true</code> if the patient domain is validated successfully; otherwise <code>false</code>.
     * @throws HL7Exception
     *             if something is wrong with HL7 message
     */
    private boolean validateDomain(ACK reply, PatientIdentifier patientId, String incomingMessageId)
        ...
        if (!domainOk) {
            ...
            HL7v231.populateERR(reply.getERR(), "PID", "1", "3", "1", "4", "204", "Unknown Key Identifier");
            return false;
        }
        return true;
    }
     ...
     *             if something is wrong with the application
     */
    private boolean validateMessage(ACK reply, HL7Header hl7Header, PatientIdentifier patientId,
            PatientIdentifier mrgPatientId, boolean isPixCreate) throws HL7Exception, PixPdqException {
        Identifier serverApplication = getServerApplication(actor.getConnection());
        Identifier serverFacility = getServerFacility(actor.getConnection());
        ...
        boolean isValidFacilityApplication = validateReceivingFacilityApplication(reply, receivingApplication,
                receivingFacility, serverApplication, serverFacility, incomingMessageId);
        ...
        if (!isPixCreate) {
            ...
            boolean isValidPid = validatePatientId(reply, patientId, hl7Header.toMessageHeader(), false,
                    incomingMessageId);
            if (!isValidPid)
                return false;
        }
     ...
     *             if something is wrong with the application
     */
    private boolean validatePatientId(ACK reply, PatientIdentifier patientId, MessageHeader header,
            boolean isMrgPatientId, String incomingMessageId) throws HL7Exception, PixPdqException {
        ...
        if (!validPatient) {
            ...
            if (isMrgPatientId) {
                ...
                HL7v231.populateERR(reply.getERR(), "PID", "1", "3", "1", "1", "204", "Unknown Key Identifier");
            }
        }
        return validPatient;
    }
    ...
    private PatientIdentifier getPatientIdentifiers(PID pid) {
        ...
        for (CX cx : cxs) {
            Identifier assignAuth = new Identifier(cx.getAssigningAuthority().getNamespaceID().getValue(),
                    cx.getAssigningAuthority().getUniversalID().getValue(),
                    cx.getAssigningAuthority().getUniversalIDType().getValue());
            Identifier assignFac = new Identifier(cx.getAssigningFacility().getNamespaceID().getValue(),
                    cx.getAssigningFacility().getUniversalID().getValue(),
                    cx.getAssigningFacility().getUniversalIDType().getValue());
            identifier.setAssigningAuthority(
                    AssigningAuthorityUtil.reconcileIdentifier(assignAuth, actor.getActorDescription(), pixAdapter));
    ...
    /**
     * Forwards this PIX Feed message to the XDS Registry in the affinity domain. The XDS registry in the affinity
     * domain is interested in patient IDs in only the global (master) assigning authority (domain). So messages for
     * non-global patient IDs are filtered out. Also, be sure to configure XDS Registry connection in the Actor
     * configuration. See the relevant actor configuration documentation.
     * 
     * @param msgIn
     *            the incoming PIX Feed message to be forwarded to the XDS Registry
     * @param patientId
     *            the ID of the patient of PIX Feed. For patient creation and update, it is the main patient ID; for
     *            patient merge, it is the surviving patient ID.
     */
    ...
    /**
     * Checks the response to the patient identity feed to ensure that it was a success.
     * 
     * @param response
     ...
     */
    private boolean processPixFeedResponse(Message response, IConnectionDescription connection)
                        ...
                        if (ce != null) {
                            ...
                            if (errorcode != null) {
                                error = "(" + errorcode.getValue() + ") "
                                        + HL7v231.getErrorString(errorcode.getValue());
                            }
                            ST text = ce.getText();
                            if (text != null)
    ...
    /**
     * Send an HL7 message over this channel to the XDS Registry and return the response.
     * 
     * @param message
    ...
    public Message sendMessage(Message message) throws IOException {
        ...
        // throw new IOException("Cannot open XDS Registry connection to \"" +
        // actor.getXdsRegistryConnection().getDescription() + "\"");
        // }
        ...
        try {
            ...
            hl7Connection = context.newClient("localhost", actor.getXdsRegistryConnection().getPort(), false);
        } catch (HL7Exception e) {
