...
public class AuditEventServiceImpl extends BaseServiceImpl implements AuditEventService
{
	...
	public AuditEventEntry saveAuditEventEntry(AuditEventEntry auditEventEntry) {
		...
		ValidationService validationService = Context.getValidationService();
		validationService.validate(auditEventEntry);
		
		return (AuditEventEntry) auditEventDao.save(auditEventEntry);
	}
	
	public AuditEventEntry saveAuditEventEntry(String auditEventTypeCode, String auditEventDescription, String entityName) {
		return saveAuditEventEntry(auditEventTypeCode, auditEventDescription, entityName, null, null);
	}
	
	public AuditEventEntry saveAuditEventEntry(String auditEventTypeCode, String auditEventDescription, String entityName, Record refRecord) {
		return saveAuditEventEntry(auditEventTypeCode, auditEventDescription, entityName, refRecord, null);
	}

	public AuditEventEntry saveAuditEventEntry(String auditEventTypeCode, String auditEventDescription, String entityName, Record refRecord, Record altRefRecord ) {
		...
		AuditEventEntry auditEventEntry = new AuditEventEntry(new java.util.Date(), auditEventType, auditEventDescription, Context.getUserContext().getUser());
		auditEventEntry.setEntityName(entityName);
		if( refRecord !=null ) {
			auditEventEntry.setRefRecordId(refRecord.getRecordId());
		}
		if( altRefRecord !=null ) {		
			auditEventEntry.setAltRefRecordId(altRefRecord.getRecordId());
		}
		
		log.debug("About to audit the event " + auditEventEntry);
		auditEventEntry = (AuditEventEntry) auditEventDao.save(auditEventEntry);
		return auditEventEntry;
	}
	
	@SuppressWarnings("unchecked")
	public List<AuditEventEntry> getAllAuditEventEntries() {
		return (List<AuditEventEntry>) auditEventDao.getAll(AuditEventEntry.class);
	}
	
	public List<AuditEventEntry> filterAuditEventEntries(Date startDate, Date endDate, List<Integer> auditEventTypeCodes) {
		return auditEventDao.filterAuditEventEntries(startDate, endDate, auditEventTypeCodes);
	}

	public int getAuditEventEntryCount(Date startDate, Date endDate, List<Integer> auditEventTypeCodes) {
		return auditEventDao.getAuditEventEntryCount(startDate, endDate, auditEventTypeCodes);
	}
	...
	public List<AuditEventEntry> filterAuditEventEntriesPaged(Date startDate, Date endDate, List<Integer> auditEventTypeCodes, int firstResult, int maxResults) {
		return auditEventDao.filterAuditEventEntriesPaged(startDate, endDate, auditEventTypeCodes, firstResult, maxResults);
	}
    ...
    public List<LoggedLink> getLoggedLinks(final int entityVersionId, final int vectorValue, final int start, final int maxResults) {
        return auditEventDao.getLoggedLinks(entityVersionId, vectorValue, start, maxResults);
    }
